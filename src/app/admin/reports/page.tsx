...
export default function ReportsPage() {
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="text-white/40 text-center py-16">Loading reports...</div>
      ) : !data ? (
        <div className="text-white/40 text-center py-16">Failed to load data</div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={card.label} className="bg-[#1C1C36] border border-white/[0.06] rounded-2xl p-5">
          ...
          <div className="grid lg:grid-cols-2 gap-5">
            {/* Orders by status */}
            <div className="bg-[#1C1C36] border border-white/[0.06] rounded-2xl p-5">
              ...
              <div className="space-y-3">
                ...
                {data.ordersByStatus.length === 0 && <p className="text-white/30 text-sm">No orders yet</p>}
              </div>
            </div>
            ...
            <div className="bg-[#1C1C36] border border-white/[0.06] rounded-2xl p-5">
              ...
              <div className="space-y-3">
                  ...
                  return (
                    <div key={item.menuItemId}>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-white/60">{item._sum.quantity ?? 0} sold</span>
                      </div>
                      <div className="w-full bg-white/5 rounded-full h-1.5">
                ...
                {data.topItems.length === 0 && <p className="text-white/30 text-sm">No sales data yet</p>}
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  <th className="text-left px-5 py-3 text-white/40 font-medium">Order</th>
                  <th className="text-left px-5 py-3 text-white/40 font-medium">Theater</th>
                  <th className="text-left px-5 py-3 text-white/40 font-medium">Seat</th>
                  ...
                  <th className="text-left px-5 py-3 text-white/40 font-medium">Time</th>
                </tr>
              </thead>
              <tbody>
                {data.recentOrders.map((order, i) => (
                  <tr key={order.orderNumber} className={`border-b border-white/[0.04] ${i === data.recentOrders.length - 1 ? "border-b-0" : ""}`}>
                    ...
                    <td className="px-5 py-3 text-white/40 text-xs">{formatDate(order.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {data.recentOrders.length === 0 && (
              <div className="text-center py-12 text-white/30">No orders in this period</div>
            )}
          </div>
        </div>
      )}
