        ...
        <div className="px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">Seat <span className="text-amber-400">{displaySeatLabel}</span></p>
            </div>
          </div>
        ...
        <div className="flex gap-2 px-4 pb-3 overflow-x-auto scrollbar-none">
          <button
            ...
            className={`whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium border transition-all
              ${activeCategory === "all" ? "bg-amber-500/20 border-amber-500/60 text-amber-400" : "border-white/10 text-white/50 hover:text-white"}`}>
            All
          </button>
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
              className={`whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium border transition-all
                ${activeCategory === cat.id ? "bg-amber-500/20 border-amber-500/60 text-amber-400" : "border-white/10 text-white/50 hover:text-white"}`}>
              {cat.icon} {cat.name}
            </button>
          ))}
        </div>
      ...
      <main className="flex-1 px-4 py-4 pb-32">
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="bg-[#141418] border border-white/10 rounded-2xl p-4 flex items-center gap-4 animate-pulse">
          ...
          <div className="space-y-3">
            {menuItems.map((item) => {
              ...
              return (
                <div key={item.id} className="bg-[#141418] border border-white/10 rounded-2xl p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      {item.isFeatured && (
                        <span className="text-[10px] text-amber-400 bg-amber-500/10 border border-amber-500/20 px-1.5 py-0.5 rounded">⭐ Popular</span>
                      )}
                    </div>
                    <p className="font-semibold text-sm">{item.name}</p>
                    {item.description && <p className="text-xs text-white/50 mt-0.5 line-clamp-1">{item.description}</p>}
                    <p className="font-bold text-amber-400 mt-1">₹{item.basePrice}</p>
                  </div>
                  ...
                  <div className="flex-shrink-0">
                    {!inCart ? (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => addItem({ id: item.id, name: item.name, price: item.basePrice, isVeg: item.isVeg, category: item.category.name })}
                          className="w-8 h-8 rounded-full bg-amber-500/20 hover:bg-amber-500 hover:text-black text-amber-400 font-bold flex items-center justify-center transition-all">+</button>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="fixed bottom-0 inset-x-0 z-50 bg-[#141418] border-t border-white/10 rounded-t-3xl p-6 max-h-[80vh] flex flex-col">
                ...
                <div key={item.id} className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="w-7 h-7 rounded-full bg-amber-500/20 text-amber-400 flex items-center justify-center font-bold">+</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-white/10 pt-4 space-y-2 mb-4">
              ...
              <div className="flex justify-between font-bold text-base pt-1 border-t border-white/10">
                <span>Total</span>
                <span className="text-amber-400">₹{cartTotal + tax + packaging}</span>
              </div>
            </div>
...
export default function MenuPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center text-white/50">Loading menu...</div>}>
      <MenuContent />
    </Suspense>
  );
}
